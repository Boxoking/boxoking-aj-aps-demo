...
import { Table, Button, Modal, Form, Input, InputNumber, Select, Space, Popconfirm, Tag, Switch, message } from "antd";
import { PlusOutlined, EditOutlined, DeleteOutlined, SearchOutlined } from "@ant-design/icons";
import { useMasterDataStore } from "../store/useMasterDataStore";
import type { PackagingMachine, PackagingSpeed, MachineStatus } from "../types";

const { Option } = Select;

const PackagingMachinePage: React.FC = () => {
  const store = useMasterDataStore();
  const [selectedMachine, setSelectedMachine] = useState<PackagingMachine | null>(null);
  const [selectedSpeed, setSelectedSpeed] = useState<PackagingSpeed | null>(null);
  const [machineModalOpen, setMachineModalOpen] = useState(false);
  const [speedModalOpen, setSpeedModalOpen] = useState(false);
  const [editingMachine, setEditingMachine] = useState<PackagingMachine | null>(null);
  const [editingSpeed, setEditingSpeed] = useState<PackagingSpeed | null>(null);
  ...
  React.useEffect(() => {
    const handleMouseMove = (e: MouseEvent) => {
      ...
      setLeftWidth(Math.min(650, Math.max(280, w)));
    };
    const handleMouseUp = () => {
  ...
  const filteredMachines = useMemo(() =>
    store.packagingMachines.filter((m) =>
      !machineSearch || m.machineName.includes(machineSearch) || m.machineId.includes(machineSearch)
    ), [store.packagingMachines, machineSearch]);

  const selectedSpeeds = store.packagingSpeeds.filter(
  ...
  const handleEditMachine = (record: PackagingMachine) => {
  ...
  const handleEditSpeed = (record: PackagingSpeed) => {
  ...
  const handleDeleteMachine = (id: string) => {
    store.deletePackagingMachine(id);
    setSelectedMachine(null);
    message.success("机台已删除");
  };

  const handleDeleteSpeed = (id: string) => {
    store.deletePackagingSpeed(id);
  ...
  const machineColumns = [
    ...
    {
      title: "手动包装",
      dataIndex: "manualPackaging",
      key: "manualPackaging",
      width: 80,
      render: (v: boolean) => (v ? <Tag color="orange">是</Tag> : <span>否</span>),
    },
    {
      title: "状态",
      dataIndex: "status",
    ...
    },
    {
      ...
      key: "actions",
      width: 100,
      render: (_: unknown, record: PackagingMachine) => (
        <Space size="small">
  ...
  const speedColumns = [
    ...
    { title: "包装机台", dataIndex: "machineName", key: "machineName", width: 140 },
    ...
    },
    {
      ...
      key: "actions",
      width: 100,
      render: (_: unknown, record: PackagingSpeed) => (
        <Space size="small">
  ...
  const machineColsWidth = 100 + 140 + 100 + 60 + 80 + 80 + 100;
  const speedColsWidth = 160 + 110 + 140 + 80 + 80 + 80 + 100;
    ...
    <div ref={containerRef} style={{ height: "100%", display: "flex", gap: 4, padding: 16, overflow: "hidden" }}>
      ...
      <div style={{ width: leftWidth, minWidth: leftWidth, background: "#fff", borderRadius: 6, padding: 12, border: "1px solid #e8e8e8", display: "flex", flexDirection: "column" }}>
        <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8, flexShrink: 0 }}>
          <span style={{ fontWeight: 600, fontSize: 15, color: "#16a34a" }}>包装机台</span>
          <Space size={4}>
      ...
      <div style={{ flex: 1, background: "#fff", borderRadius: 6, padding: 12, border: "1px solid #e8e8e8", display: "flex", flexDirection: "column", overflow: "hidden" }}>
        <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8, flexShrink: 0 }}>
          <span style={{ fontWeight: 600, fontSize: 15 }}>
            {selectedMachine ? `${selectedMachine.machineName} — 可包装品相` : "请选择机台"}
          </span>
          {selectedMachine && (
            <Space size={4}>
      ...
      <Modal
        title={editingMachine ? "修改包装机台" : "新建包装机台"}
      ...
      >
        <Form form={machineForm} layout="vertical" size="small">
          ...
          <Form.Item name="manualPackaging" label="手动包装" valuePropName="checked">
            <Switch />
          </Form.Item>
      ...
      <Modal
        title={editingSpeed ? "修改包装速度" : "新增品相包装速度"}
        open={speedModalOpen}
      ...
      >
        <Form form={speedForm} layout="vertical" size="small">
          ...
          <Form.Item name="machineId" label="包装机台" hidden><Input /></Form.Item>
          <Form.Item name="machineName" label="包装机台名称" hidden><Input /></Form.Item>
          <Space>
...
export default PackagingMachinePage;
